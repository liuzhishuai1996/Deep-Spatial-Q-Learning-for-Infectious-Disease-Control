...
from src.estimation.q_functions.embedding4 import tune_ggcn
from src.estimation.q_functions.embedding3 import tune_ggcn1
...
def one_step_policy(**kwargs):
  ...
  if env.learn_embedding: 
    predictor, _ = tune_ggcn(env.X_raw, env.y, env.adjacency_list, n_epoch=200, neighbor_order=1, num_settings_to_try=10)
    def qfn(a_):
...
def two_step(**kwargs):
  ...
  if env.learn_embedding:
    ...
    for i in range(len(backup)):
      y.append(backup[i])
    predictor1, _ = tune_ggcn1(env.X_raw[:-1], y, env.adjacency_list, n_epoch=200, neighbor_order=1, num_settings_to_try=10)
    def qfn(a_):
      #infections = env.Y[-1, :]
...
def three_step(**kwargs):
  ...
  if env.learn_embedding:
    ...
    for i in range(len(backup)):
      y.append(backup1[i])
    predictor2, _ = tune_ggcn1(env.X_raw[:-1], y, env.adjacency_list, n_epoch=200, neighbor_order=1, num_settings_to_try=10)
